...
def method_1():
    ...
    line1, = ax1.plot(x, x ** 1)
    line2, = ax2.plot(x, x ** 2)

# figure / subplot
# 缺點：如果有多個 fig 就認不出是哪個？
def method_2():
    fig = plt.figure(figsize=(8,6))
    ax4 = plt.subplot(2, 3, 4)                       # 2x3 子圖中，僅先創建第 4 個子圖，等價於 fig.add_subplot(234)
...
+ **模組** `animation`
    ...
    + **類別** `FuncAnimation`
      ...
      def init():
          ...
          ax.set_ylim(-1.5, 1.5)
          return line, # 回傳 tuple[Artist]

      # 更新函數，使用 blitting 只更新需要的部分
      def update(frame):
          xdata.append(frame)
      ...
      # 創建動畫
      ani = animation.FuncAnimation(fig, func=update, frames=np.linspace(0, 2*np.pi, 128), init_func=init, blit=True)
      # FuncAnimation
          # (
          #   #參數 fig
          #      + 畫布
          ...
          #      + 每個幀都會調用的函數
          #      - 必須簽名：
          #      - def func(frame, *fargs) -> tuple[Artist] | 容器裝著的 Artists
          #         - frame: 第一參數固定是被傳入的一幀的數據
          #         - *fargs: 其他隨意
          #      - 可使用 partial 柯里化函數預先固定其他參數
          ...
          #      + 在第一幀開始前調用的函數
          #      - 必須簽名：
          #      - def init_func() -> tuple[Artist] | 容器裝著的 Artists
          #
          #   #參數 fargs 
          #      + 餵給 func 的額外參數 (不過 mat 比較推薦用 partial)
          #
          #   #參數 save_count
          ...
          #      - Blitting 技術的目的是僅重新繪製需要更改的部分，而不是整個畫布，從而提高了繪製效率。
          #      ⚠️ 如果 blit 設為 True，func / init_func 必須回傳所有被修改或創建的 artists 可迭代物件。
...
+ **藝術家** `artist`
    ...
    > 數據型的藝術家應該都是使用 .set_data 更新？
    
    + **簡介**
      > 在 Matplotlib 中，\
      > 藝術家 (Artist) 是指一個由圖形元素 (例如線條、點、文本等) 構成的物件。\
      > 你可以將不同的藝術家添加到圖形中，這些藝術家共同構成了圖形的各個元素。\
      > 在動畫中，FuncAnimation 函數期望的返回值就是一個 `tuple[Artist] | Container[Artist]`\
    ...
    + **2D**
      ...
      # bar
          bars = ax.bar()
          bars = ax.barh()
          bars: BarContainer[Rectangle] # 容器
          bar: Rectangle # 藝術家
      
      # pie
          wedges, texts, autotexts = ax.pie()
          wedges: list[Wedge]   # list[藝術家]
          texts: list[Text]     # list[藝術家]
          autotexts: list[Text] # list[藝術家]
      ...
      # stackplot
          stacks = ax.stackplot()
          stacks: list[PolyCollection] # list[藝術家]
      ...
      # hist
          ...
          heights, bins = np.histogram() # 把 data 丟進去，幫你算好 heights, bins，更新藝術家時使用
          heights: list[float]
          bins: list[float]
          bars: BarContainer[Rectangle] # 容器
          bar: Rectangle # 藝術家
